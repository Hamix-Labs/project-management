...
func TestRun_generatedFilesExemptFromFetchRule(t *testing.T) {
	root := newRepo(t)
	writeRepoFile(t, root, "web/src/lib/tiptap-collab-utils.ts",
		"// Code generated by @tiptap/cli. DO NOT EDIT.\nexport const load = () => fetch(\"/token\");\n")

	if _, err := runCheck(t, root); err != nil {
		t.Fatalf("run returned %v, want nil for generated CLI files", err)
	}
}
...
func TestCheckFileSizes_generatedTsExempt(t *testing.T) {
	root := newRepo(t)
	writeRepoFile(t, root, "web/src/components/tiptap-ui/huge.tsx",
		"// Code generated by @tiptap/cli. DO NOT EDIT.\n\nexport const X = 1\n"+strings.Repeat("export const _ = 1\n", 400))

	out, err := runCheck(t, root)
	if err != nil {
		t.Fatalf("run returned %v, want nil for generated TSX\noutput:\n%s", err, out)
	}
	if strings.Contains(out, "SIZE (red)") {
		t.Fatalf("generated TSX should be skipped entirely:\n%s", out)
	}
}
