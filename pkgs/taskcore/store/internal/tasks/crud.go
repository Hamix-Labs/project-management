...
func Update(ctx context.Context, db *gorm.DB, id string, in UpdateInput, by domain.Actor) (*domain.Task, domain.Status, error) {
	...
	if in.Title == nil && in.InitialPrompt == nil && in.Status == nil && in.Priority == nil && in.Project == nil && in.PickupNotBefore == nil && in.CursorModel == nil && in.Tags == nil && in.Milestone == nil && in.Gate == nil && in.DependsOn == nil && in.PendingRetry == nil && !in.ClearPendingRetry && in.WorktreeID == nil && in.PullRequestURL == nil {
...
func buildCreateTaskFromInput(in CreateInput, by domain.Actor) (t *domain.Task, title string, st domain.Status, err error) {
	...
	t = &domain.Task{
		...
		Priority:        pr,
		ProjectID:       projectID,
		RepositoryID:    normalizeOptionalID(in.RepositoryID),
		Runner:          runner,
