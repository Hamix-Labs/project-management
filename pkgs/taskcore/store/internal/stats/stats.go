...
var allPhases = []cyclesdomain.Phase{
	cyclesdomain.PhaseExecute,
	cyclesdomain.PhaseVerify,
}
...
func Get(ctx context.Context, db *gorm.DB) (TaskStats, error) {
	...
	out := TaskStats{
		...
		Phases: PhaseStats{
			ByPhaseStatus: make(map[cyclesdomain.Phase]map[cyclesdomain.PhaseStatus]int64, len(allPhases)),
		},
		Runner: RunnerStats{
	...
	for _, p := range allPhases {
		out.Phases.ByPhaseStatus[p] = map[cyclesdomain.PhaseStatus]int64{}
	}
	...
	for _, p := range phaseRows {
		...
		if !ok {
			...
			bucket = map[cyclesdomain.PhaseStatus]int64{}
