...
type TaskCycle struct {
	...
	Status        cyclesdomain.CycleStatus `gorm:"not null;index;check:chk_task_cycles_status,status IN ('running','succeeded','failed','aborted')"`
	...
	Task *TaskRow `gorm:"foreignKey:TaskID;references:ID;constraint:OnDelete:CASCADE"`
}
