...
The public package is a **facade**: every `*Store` method in a `facade_*.go` file delegates to a per-domain package under [`internal/`](./internal) or an extracted bounded context (notably [`pkgs/taskcore/store`](../../taskcore/store)). Public types (`CreateTaskInput`, `TaskNode`, …) are Go type aliases over the internal or taskcore definitions, so external callers stay unchanged across reshuffles. Cross-domain transactions are composed by calling the exported `…InTx` helpers from sibling internal packages inside one `*gorm.DB.Transaction(...)`.
...
| Tasks — CRUD, lists & trees | `facade_tasks.go` | `facade_tasks_test.go` | [`pkgs/taskcore/store`](../../taskcore/store) | Embeds `taskcore *taskcorestore.Store`; readiness via [`pkgs/tasks/scheduling/`](../scheduling/). |
| Stats | `facade_stats.go` | — | [`pkgs/taskcore/store`](../../taskcore/store) | `TaskStats`, cycle failure scans. |
...
| Agent ready queue | `facade_ready.go` | `facade_ready_test.go`, `scheduling_parity_test.go` | [`pkgs/taskcore/store`](../../taskcore/store) | `ListReadyTaskQueueCandidates`, `ListReadyTasksUserCreated`. |
| Health | `facade_health.go` | `facade_health_test.go` | [`pkgs/taskcore/store`](../../taskcore/store) | `Ping`, `Ready`. |
| Dev simulation | `facade_devmirror.go` | `facade_devmirror_test.go` | [`pkgs/taskcore/store`](../../taskcore/store) | `ApplyDevTaskRowMirror`, `ListDevsimTasks`. |
