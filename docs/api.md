...
| POST | `/tasks/{id}/open-pr` | Human approve-and-open-PR from agent-complete. Requires `X-Actor: user`, `status=review`, and the task must be the **worktree root** (`worktree_root_task_id`). Optional body `{ parent_cycle_id? }`. Queues `pending_retry` kind `open_pr` (mode `resume`), transitions `review→ready`, appends `approval_granted` and `open_pr_requested`. The open-pr run publishes the worktree GitHub stack via `gh stack submit`. Non-root tasks receive `400`. See [ADR-0082](./adr/ADR-0082-human-approval-gate.md), [ADR-0097](./adr/ADR-0097-worktree-stack-layers.md). |
| POST | `/tasks/{id}/approve` | Human completion. Requires `X-Actor: user` and complete checklist. From `pr_ready` → `done` (root / post-PR mark-done). From `review` → `done` only for **non-root** stack layers (stack PRs are published by the root open-pr). Publishes `task_updated` and unblocks dependents. Free-form `PATCH { status: done }` is rejected (`400`). See [ADR-0082](./adr/ADR-0082-human-approval-gate.md). |
...
| GET | `/repo/files?worktree_id=` | `{ paths: string[], truncated, source }`. Every referenceable file under the worktree, sorted, slash-separated, with no query filter — the client caches this and ranks locally for `@`-mentions. `source: "git"` means the list came from `git ls-files --cached --others --exclude-standard`, so nested `.gitignore`, `.git/info/exclude`, and `core.excludesFile` all apply and tracked-but-ignored files are still listed. `source: "walk"` means the worktree is not a git work tree (plain directory or bare repo) and a directory walk with a fixed skip list was used instead, where ignore rules do not apply. `truncated: true` when the repository exceeds 50 000 paths. Requires `worktree_id`. |
