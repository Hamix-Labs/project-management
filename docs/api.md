...
| POST | `/tasks/{id}/approve` | Human completion after agent-complete. Requires `X-Actor: user` and `status=review`. Checklist must be complete. Transitions `review→done`, appends `status_changed`, `approval_granted`, and `on_task_done` (commits from latest succeeded cycle). Publishes `task_updated` and unblocks dependents. Free-form `PATCH { status: done }` is rejected (`400`). See [ADR-0082](./adr/ADR-0082-human-approval-gate.md). |
| POST | `/tasks/{id}/polish` | Human rework from `review`. Body `{ instructions, parent_cycle_id? }`. Requires `X-Actor: user`, non-empty `instructions`. Resolves `parent_cycle_id` to the latest **succeeded** cycle when omitted. Sets `pending_retry` with `kind=polish`, `mode=resume`, and instructions; transitions `review→ready`. Idempotent when the same polish intent is already queued; `409` on conflicting pending intent. Appends `task_polish_requested`. Worker resumes the parent execute Cursor session, always runs execute→verify, then returns to `review`. |
