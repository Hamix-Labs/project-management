...
| GET | `/tasks/activity` | Paginated cross-task activity feed. Fixed type filter: `status_changed`, `phase_failed`, `approval_granted`. Query params: `limit` (1–200, default 50), `offset` (≥ 0), `since` (optional RFC3339 lower bound on `at`). Ordered newest-first (`at DESC, seq DESC`). Envelope: `{ total, limit, offset, events: [{ task_id, seq, at, type, by, data, task_title? }] }`. `Cache-Control: no-store`. |
