...
- **Pool sizing** — `app_settings.agent_task_parallelism` (any integer ≥ 1, default 8) sets concurrent harness run slots. Effective parallelism is bounded by distinct worktrees with ready tasks, not by queue depth alone.
...
10. **Agent worker is single-process.** Pool size is Settings-configurable (`agent_task_parallelism`, default 8). No retry/backoff; one attempt per task. Runs on the same worktree share one directory and serialize via `WorktreeGate`.
