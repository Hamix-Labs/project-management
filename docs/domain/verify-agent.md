...
How the harness accepts done criteria after execute: claim-only (`execute_claim`) for all criteria, including those with operator `verify_commands` the execute agent was instructed to self-check ([ADR-0091](../adr/ADR-0091-execute-owns-verify-commands.md)).

> **Product default (ADR-0091)** — Every active criterion with `claimed_done: true` is accepted as `verified_by=execute_claim`. There is no worker shell re-run and no PhaseVerify Cursor pass. Criteria with `verify_commands` are listed in the **execute** prompt so the agent can run them before claiming. `claimed_done: false` fails the cycle (`agent_self`). Operators recover via **Retry** / **Start over**.
...
Historical cycles may still show `phase=verify`, `verified_by=execute_agent`, and `command_runs` from the pre-ADR-0091 path.
...
- [ADR-0091](../adr/ADR-0091-execute-owns-verify-commands.md)
