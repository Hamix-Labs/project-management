...
When a task row is committed in **`status=ready`**, the store may push a **full `domain.Task` snapshot** into an optional in-process queue. A **worker pool** (default 150 slots, `app_settings.agent_task_parallelism`) shares one queue and runs [harness.md](./harness.md) admission (`Run` or `Resume`) per dequeued task.
...
Shared consumers: [`worker.Pool.Run`](../../pkgs/agents/worker/pool.go) starts N goroutines (default 150), each looping on `queue.Receive(ctx)` against one `MemoryQueue` and one [`WorktreeGate`](../../pkgs/agents/worker/worktree_gate.go).
...
| `agent_task_parallelism` | app_settings | Max parallel tasks across different worktrees (default 150); [configuration.md](../configuration.md) |
