...
4. **Fetch before allocate** — Allocation always `git fetch`es `origin` first, then starts the new branch from `origin/<defaultBranch>`. Fail closed if fetch/auth fails; do not silently use a stale local main. (**Timing:** as of [ADR-0083](./ADR-0083-async-task-worktree-provision.md), allocate runs asynchronously after `POST /tasks` returns; it remains mandatory before agent pickup.)
