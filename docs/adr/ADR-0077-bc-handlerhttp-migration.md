...
3. **BC `http_helpers.go`** keeps only BC-specific path parsers, query parsers, and error mappers that are not generic (e.g. `repoErrUserMessage`, projects/settings `writeStoreError` for BC domain sentinels, gitinventory `GitErrHTTP` / `WriteGitStoreError`). Shared `DebugHTTPRequest` / `DebugHTTPOut` / `TruncateRunes` live in `handlerhttp/httplog.go`; BC packages may keep thin wrappers. Settings retains a thin `debugHTTPRequest` that preserves its `"http request"` slog message while sharing query-byte truncation.
4. **gitinventory exception:** `handlerhttp.WriteStoreError` delegates git-coded errors to `gitinventory/handler.WriteGitStoreError`, creating an import cycle if git handlers import `handlerhttp`. Git handlers retain local thin JSON helpers in `http_helpers.go` that call `pkgs/tasks/apijson` directly (same wire shape, documented exception). Shared `InvalidInputDetail` (configurable domain prefixes) also lives in `apijson` so gitinventory can use the same helper without importing `handlerhttp`.
