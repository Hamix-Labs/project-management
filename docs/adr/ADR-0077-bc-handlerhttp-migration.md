...
3. **BC `http_helpers.go`** keeps only BC-specific path parsers, query parsers, and error mappers that are not generic (e.g. `repoErrUserMessage` via `apijson.UserFacingMessage`, projects/settings `writeStoreError` for BC domain sentinels, gitinventory `GitErrHTTP` / `WriteGitStoreError`). Shared `DebugHTTPRequest` / `DebugHTTPOut` / `TruncateRunes` live in `handlerhttp/httplog.go` and are called directly from handlers (no per-BC thin wrappers). Settings uses the shared `"http.io"` slog message.
