...
const TABLE_PAGE_FRAGMENT_GAP_PX = 30

function getTablePageFragmentRect(
  tbody: HTMLTableSectionElement,
  reference: Element
): DOMRect | null {
  const referenceRow = reference.closest("tr")
  if (!referenceRow || !tbody.contains(referenceRow)) {
    return null
  }

  const rows = tbody.rows
  let referenceIndex = referenceRow.sectionRowIndex
  if (referenceIndex < 0 || rows[referenceIndex] !== referenceRow) {
    referenceIndex = Array.prototype.indexOf.call(rows, referenceRow)
  }
  ...
  const referenceRect = referenceRow.getBoundingClientRect()
  let top = referenceRect.top
  let bottom = referenceRect.bottom

  let first = referenceIndex
  let firstRect = referenceRect
  while (first > 0) {
    const previousRow = rows[first - 1]!
    const previousRect = previousRow.getBoundingClientRect()
    if (firstRect.top - previousRect.bottom > TABLE_PAGE_FRAGMENT_GAP_PX) {
      break
    }

    first -= 1
    firstRect = previousRect
    top = Math.min(top, previousRect.top)
    bottom = Math.max(bottom, previousRect.bottom)
  }

  let last = referenceIndex
  let lastRect = referenceRect
  while (last < rows.length - 1) {
    const nextRow = rows[last + 1]!
    const nextRect = nextRow.getBoundingClientRect()
    if (nextRect.top - lastRect.bottom > TABLE_PAGE_FRAGMENT_GAP_PX) {
      break
    }

    last += 1
    lastRect = nextRect
    top = Math.min(top, nextRect.top)
    bottom = Math.max(bottom, nextRect.bottom)
  }
  ...
  const containerRect = tbody
    .closest(".table-container")
    ?.getBoundingClientRect()
  const left = containerRect?.left ?? referenceRect.left
  const width =
...
/**
 * Measure a table's bounds from its `<tbody>`.
 *
 * With the Pages extension active, `table` and `tbody` render as
 * `display: contents` (so rows can flow through the paginated layout) and
 * have no layout box — `getBoundingClientRect()` returns a zero rect. In
 * that case fall back to the node view's `.table-container`. When a
 * reference cell/row is provided, return only the current page fragment so
 * handles stay anchored to the visible table segment instead of the whole
 * multi-page bounding box.
 */
export function getTableBodyRect(
  tbody: HTMLTableSectionElement,
  ...
  if (reference) {
    const fragmentRect = getTablePageFragmentRect(tbody, reference)
    if (fragmentRect) {
      return fragmentRect
    }
  }
  ...
  return container ? container.getBoundingClientRect() : rect
}
