...
func mustStartCycle(t *testing.T, baseURL, taskID string) string {
	t.Helper()
	res, raw := handlertest.DoCyclesRequest(t, http.MethodPost, baseURL+"/tasks/"+taskID+"/cycles", `{}`)
...
func TestHTTP_postTaskCycle_400ErrorStrings(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			res, raw := handlertest.DoCyclesRequest(t, http.MethodPost, srv.URL+"/tasks/"+taskID+"/cycles", tc.body)
