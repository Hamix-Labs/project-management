...
func TestHTTP_patch_task_event_user_response(t *testing.T) {
	...
	approvalSeq := handlertest.AppendApprovalRequestedEvent(t, st, ctx, created.ID)
	...
	reqAgent, err := http.NewRequest(http.MethodPatch, srv.URL+"/tasks/"+created.ID+"/events/"+handlertest.FormatEventSeq(approvalSeq), strings.NewReader(`{"user_response":"Please confirm"}`))
	if err != nil {
		t.Fatal(err)
	}
	...
	reqOK, err := http.NewRequest(http.MethodPatch, srv.URL+"/tasks/"+created.ID+"/events/"+handlertest.FormatEventSeq(approvalSeq), strings.NewReader(`{"user_response":"LGTM"}`))
	if err != nil {
		t.Fatal(err)
	}
...
func TestHTTP_get_task_event(t *testing.T) {
	...
	longSeq := strings.Repeat("1", handlertest.MaxTaskEventSeqParamBytes+1)
...
func TestHTTP_task_events_query_validation_error_messages(t *testing.T) {
	...
	res, err := http.Post(srv.URL+"/tasks", "application/json", strings.NewReader(handlertest.WithCreateChecklistForURL(srv.URL, `{"title":"e","priority":"medium"}`)))
	if err != nil {
		t.Fatal(err)
	}
	b, err := io.ReadAll(res.Body)
	...
	long := strings.Repeat("1", handlertest.MaxTaskEventSeqParamBytes+1)
