...
// Mirrors handlertest.GetTask/handlertest.DeleteTask in the surrounding contract suites: returns the
...
func TestHTTP_patchEvent_pathSegmentGuard(t *testing.T) {
	...
	cases := []struct {
		...
		{"overlongSeq", id, strings.Repeat("9", handlertest.MaxTaskEventSeqParamBytes+1), "seq too long"},
...
func TestHTTP_patchEvent_threadFullIs400(t *testing.T) {
	...
	res, raw := patchEventUserResponse(t, srv.URL, id, handlertest.FormatEventSeq(approvalSeq), `{"user_response":"one too many"}`, "")
	if res.StatusCode != http.StatusBadRequest {
		t.Fatalf("status %d (want 400) body=%s", res.StatusCode, raw)
	}
	var errBody handlertest.JSONErrorBody
...
func TestHTTP_patchEvent_publishesTaskEventChanged(t *testing.T) {
	...
	res, raw := patchEventUserResponse(t, srv.URL, id, handlertest.FormatEventSeq(approvalSeq), `{"user_response":"go"}`, "")
	if res.StatusCode != http.StatusOK {
		t.Fatalf("status %d body=%s", res.StatusCode, raw)
	}

	got := handlertest.SummarizeSSEEvents(handlertest.DrainSSE(t, ch, 1, 2*time.Second))
	handlertest.MustEqualEvents(t, "PATCH /tasks/{id}/events/{seq}", got, []string{fmt.Sprintf("task_event_changed:%s/%d", id, approvalSeq)})
}
