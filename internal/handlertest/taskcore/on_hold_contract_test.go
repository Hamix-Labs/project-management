...
func TestHTTP_createTask_onHoldStatusAllowed(t *testing.T) {
	...
	res, raw := handlertest.PostCreate(t, srv.URL, handlertest.WithCreateChecklist(`{"title":"hold","priority":"medium","status":"on_hold"}`))
...
func TestHTTP_patchTask_onHoldRoundTrip(t *testing.T) {
	...
	id := handlertest.MustCreateTask(t, srv.URL, `{"title":"toggle","priority":"medium"}`)

	// ready → on_hold
	res, raw := handlertest.PatchTask(t, srv.URL, id, `{"status":"on_hold"}`)
	...
	// on_hold → ready
	res, raw = handlertest.PatchTask(t, srv.URL, id, `{"status":"ready"}`)
	if res.StatusCode != http.StatusOK {
