...
// handlertest.DeleteTask centralizes the documented DELETE /tasks/{id} round-trip so the
// table-driven 400/404 subtests stay focused on the assertion side. Mirrors
// the handlertest.PatchTask helper in handler_http_patch_contract_test.go.
...
func TestHTTP_deleteTask_204EmptyBody(t *testing.T) {
	...
	id := handlertest.MustCreateTask(t, srv.URL, `{"title":"to-delete","priority":"medium"}`)
	...
	res2, raw2 := handlertest.DeleteTask(t, srv.URL, id)
...
func TestHTTP_deleteTask_pathSegmentGuard(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			res, raw := handlertest.DeleteTask(t, srv.URL, tc.path)
...
func TestHTTP_deleteTask_unknownIDIs404(t *testing.T) {
	...
	res, raw := handlertest.DeleteTask(t, srv.URL, "00000000-0000-0000-0000-0000000000ff")
	if res.StatusCode != http.StatusNotFound {
...
func TestHTTP_deleteTask_publishesTaskDeleted(t *testing.T) {
	...
	t.Run("emitsTaskDeleted", func(t *testing.T) {
		id := handlertest.MustCreateTask(t, srv.URL, `{"title":"orphan","priority":"medium"}`)
		ch, unsub := hub.Subscribe()
		defer unsub()

		if res, raw := handlertest.DeleteTask(t, srv.URL, id); res.StatusCode != http.StatusNoContent {
		...
		handlertest.MustEqualEvents(t, "DELETE /tasks/{id}", got,
