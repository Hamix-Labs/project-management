...
func TestHTTP_createTask_400ErrorStrings(t *testing.T) {
	...
	cases := []struct {
		...
		{"titleWhitespace", handlertest.WithCreateChecklist(`{"title":"   ","priority":"medium"}`), "title required"},
		{"priorityMissing", `{"title":"ok","checklist_items":[{"text":"criterion"}]}`, "priority required"},
		{"priorityEmpty", handlertest.WithCreateChecklist(`{"title":"ok","priority":""}`), "priority required"},
		{"priorityInvalid", handlertest.WithCreateChecklist(`{"title":"ok","priority":"nope"}`), "priority"},
		{"statusInvalid", handlertest.WithCreateChecklist(`{"title":"ok","priority":"medium","status":"nope"}`), "status"},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			case "checklistMissing", "checklistEmpty", "checklistWhitespaceOnly":
				body = handlertest.WithCreateGitBinding(srv.URL, tc.body)
			default:
				body = handlertest.WithCreateChecklistForURL(srv.URL, tc.body)
			}
...
func TestHTTP_createTask_409DuplicateID(t *testing.T) {
	...
	res1, raw1 := handlertest.PostCreate(t, srv.URL, handlertest.WithCreateChecklist(`{"id":"`+id+`","title":"first","priority":"medium"}`))
	if res1.StatusCode != http.StatusCreated {
		t.Fatalf("first create status %d body=%s", res1.StatusCode, raw1)
	}

	res2, raw2 := handlertest.PostCreate(t, srv.URL, handlertest.WithCreateChecklist(`{"id":"`+id+`","title":"second","priority":"medium"}`))
...
func TestHTTP_createTask_defaults(t *testing.T) {
	...
	t.Run("statusOmittedDefaultsToReady", func(t *testing.T) {
		res, raw := handlertest.PostCreate(t, srv.URL, handlertest.WithCreateChecklist(`{"title":"d1","priority":"medium"}`))
...
func TestHTTP_createTask_doneStatusWithCriteriaRejected(t *testing.T) {
	...
	res, raw := handlertest.PostCreate(t, srv.URL, handlertest.WithCreateChecklist(`{"title":"born-done","priority":"medium","status":"done"}`))
	if res.StatusCode != http.StatusBadRequest {
...
func TestHTTP_createTask_201ResponseShape(t *testing.T) {
	...
	res, raw := handlertest.PostCreate(t, srv.URL, handlertest.WithCreateChecklist(`{"title":"leaf","priority":"medium"}`))
	if res.StatusCode != http.StatusCreated {
		t.Fatalf("status %d body=%s", res.StatusCode, raw)
	}
...
func TestHTTP_createTask_acceptsPickupNotBefore_overrideGlobalDelay(t *testing.T) {
	...
	body := handlertest.WithCreateChecklist(`{"title":"explicit","priority":"medium","pickup_not_before":"` + want.Format(time.RFC3339) + `"}`)
	res, raw := handlertest.PostCreate(t, srv.URL, body)
	if res.StatusCode != http.StatusCreated {
...
func TestHTTP_createTask_rejectsBadPickupNotBefore(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:       "malformed",
			body:       handlertest.WithCreateChecklist(`{"title":"x","priority":"medium","pickup_not_before":"yesterday"}`),
			wantSubstr: "pickup_not_before must be RFC3339",
		},
		{
			name:       "pre2000Sentinel",
			body:       handlertest.WithCreateChecklist(`{"title":"x","priority":"medium","pickup_not_before":"1999-12-31T23:59:59Z"}`),
			wantSubstr: "pickup_not_before must be on or after 2000-01-01",
		},
		{
			name:       "emptyStringOnCreate",
			body:       handlertest.WithCreateChecklist(`{"title":"x","priority":"medium","pickup_not_before":""}`),
			wantSubstr: "pickup_not_before must not be empty on create",
		},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			res, raw := handlertest.PostCreate(t, srv.URL, tc.body)
			if res.StatusCode != http.StatusBadRequest {
				t.Fatalf("status %d (want 400) body=%s", res.StatusCode, raw)
			}
			var errBody handlertest.JSONErrorBody
...
func TestHTTP_createTask_pickupNotBefore_pastIsAllowed(t *testing.T) {
	...
	body := handlertest.WithCreateChecklist(`{"title":"past","priority":"medium","pickup_not_before":"` + past.Format(time.RFC3339) + `"}`)
	res, raw := handlertest.PostCreate(t, srv.URL, body)
	if res.StatusCode != http.StatusCreated {
...
func TestHTTP_createTask_publishesTaskCreated(t *testing.T) {
	...
	res, raw := handlertest.PostCreate(t, srv.URL, handlertest.WithCreateChecklist(`{"title":"child","priority":"medium"}`))
	if res.StatusCode != http.StatusCreated {
		t.Fatalf("status %d body=%s", res.StatusCode, raw)
	}
	...
	handlertest.MustEqualEvents(t, "POST /tasks", got, []string{"task_created:" + task.ID})
}
...
func TestHTTP_createTask_checklistItemsPersisted(t *testing.T) {
	...
	res, raw := handlertest.PostCreateRaw(t, srv.URL, handlertest.WithCreateGitBinding(srv.URL, `{"title":"with-criteria","priority":"medium","checklist_items":[{"text":"Ship feature"},{"text":"Add tests"}]}`))
	if res.StatusCode != http.StatusCreated {
		t.Fatalf("status %d body=%s", res.StatusCode, raw)
	}
	var task taskcoredomain.Task
...
func TestHTTP_createTask_checklistVerifyCommandsPersisted(t *testing.T) {
	...
	body := handlertest.WithCreateGitBinding(srv.URL, `{"title":"with-verify","priority":"medium","checklist_items":[{"text":"Ship feature","verify_commands":[{"command":"go test ./...","expected_outcome":"pass"}]}]}`)
	res, raw := handlertest.PostCreateRaw(t, srv.URL, body)
	if res.StatusCode != http.StatusCreated {
		t.Fatalf("status %d body=%s", res.StatusCode, raw)
	}
