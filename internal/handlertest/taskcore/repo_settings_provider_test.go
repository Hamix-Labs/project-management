...
func TestHTTP_repoRoutes_followRegisteredWorktree(t *testing.T) {
	...
	res, err := http.Get(srv.URL + handlertest.RepoSearchWithWorktree(worktreeID, "settings_repo"))
	if err != nil {
		t.Fatal(err)
	}
	defer res.Body.Close()
...
func TestHTTP_createTask_skipsMentionValidationWhenNoMentions(t *testing.T) {
	...
	res, err := http.Post(srv.URL+"/tasks", "application/json",
		strings.NewReader(handlertest.WithCreateChecklistForURL(srv.URL, `{"title":"t","initial_prompt":"plain prompt","priority":"medium"}`)))
	if err != nil {
		t.Fatal(err)
	}
	defer res.Body.Close()
...
func TestHTTP_createTask_mentionRejectsMissingFile(t *testing.T) {
	dir := t.TempDir()
	srv, _, _, _ := handlertest.NewBoundRepoServerWithStore(t, dir)
	defer srv.Close()

	res, err := http.Post(srv.URL+"/tasks", "application/json",
		strings.NewReader(handlertest.WithCreateChecklistForURL(srv.URL, `{"title":"t","initial_prompt":"@nope.txt","priority":"medium"}`)))
	if err != nil {
		t.Fatal(err)
	}
	defer res.Body.Close()
...
func TestHTTP_repoRoutes_unknownWorktree_returns404(t *testing.T) {
	...
	res, err := http.Get(srv.URL + handlertest.RepoSearchWithWorktree("00000000-0000-0000-0000-000000000099", "x"))
	if err != nil {
		t.Fatal(err)
	}
	defer res.Body.Close()
	...
	if body.Reason != handler.RepoReasonWorktreeNotFound {
