...
func TestUserCreatedTaskEnqueuesForAgents(t *testing.T) {
	...
	srv := handlertest.NewCreateServerFromStore(t, st)
	t.Cleanup(srv.Close)

	res, err := http.Post(srv.URL+"/tasks", "application/json", strings.NewReader(handlertest.WithCreateChecklistForURL(srv.URL, `{"title":"from-user","priority":"medium"}`)))
	if err != nil {
		t.Fatal(err)
	}
...
func TestAgentActorCreateEnqueuesWhenReady(t *testing.T) {
	...
	req, err := http.NewRequest(http.MethodPost, srv.URL+"/tasks", strings.NewReader(handlertest.WithCreateChecklistForURL(srv.URL, `{"title":"from-agent","priority":"medium"}`)))
	if err != nil {
		t.Fatal(err)
	}
