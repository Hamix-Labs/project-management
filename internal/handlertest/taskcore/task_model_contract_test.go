...
func TestHTTP_createTask_tagsMilestoneDependsOn(t *testing.T) {
	...
	dep := handlertest.MustCreateTask(t, srv.URL, `{"title":"dep","priority":"medium","status":"ready"}`)
	res, raw := handlertest.PostCreate(t, srv.URL, handlertest.WithCreateChecklist(`{
		"title":"blocked",
		"priority":"medium",
		"status":"ready",
		"tags":["infra","api"],
...
func TestHTTP_patchTask_depCycleRejected(t *testing.T) {
	...
	a := handlertest.MustCreateTask(t, srv.URL, `{"title":"a","priority":"medium"}`)
	b := handlertest.MustCreateTask(t, srv.URL, `{"title":"b","priority":"medium"}`)
	res, raw := handlertest.PatchTask(t, srv.URL, a, `{"depends_on":["`+b+`"]}`)
	if res.StatusCode != http.StatusOK {
		t.Fatalf("patch a dep b: %d %s", res.StatusCode, raw)
	}
	res, raw = handlertest.PatchTask(t, srv.URL, b, `{"depends_on":["`+a+`"]}`)
...
func TestHTTP_patchTaskGate_release(t *testing.T) {
	...
	res, raw := handlertest.PatchTask(t, srv.URL, id, `{"gate":null}`)
	if res.StatusCode != http.StatusOK {
	...
	id2 := handlertest.MustCreateTask(t, srv.URL, `{
		"title":"gated2",
		"priority":"medium",
